...
let FFT = function (timeSignal) {
     ...
     */
    if (timeSignal.length == 4)
        return FFT4(timeSignal);
    let size = timeSignal.length, signalEven = [], signalOdd = [];
    ...
    let freqEven = FFT(signalEven), freqOdd = FFT(signalOdd), freqSignal = [];
...
let parseReal = function (realSignal) {
...
let zeroPadding = function (realSignal, fftSize) {
...
Signal.fft = function (realSignal, fftSizeRef) {
    /*
    ...
    */
    ...
    if (fftSizeRef == undefined) {
        ...
        if (Math.log2(fftSizeRef) % 1 != 0)
            fftSize = Math.pow(2, Math.ceil(Math.log2(fftSizeRef)));
    }
    else {
        if (fftSizeRef < realSignal.length)
            throw 'fftSize is smaller than size of the signal';
        if (Math.log2(fftSizeRef) % 1 != 0)
            throw 'FFT size is not a power of 2';
        fftSize = fftSizeRef;
    }
    if (realSignal.length == 2 && fftSize == 2)
        return FFT2(parseReal(realSignal));
    if (realSignal.length == fftSize)
        return FFT(parseReal(realSignal));
    return FFT(parseReal(zeroPadding(realSignal, fftSize)));
};
Signal.ifft = function (freqSignal, fftSize) {
    let result = Signal.fft(freqSignal, fftSize);
    for (let i = 0; i < freqSignal.length; i++)
        result[i] = result[i].multiply(1 / fftSize);
    return result;
};
Signal.get = {

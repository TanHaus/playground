...
Statistics.var = function (list, sample = true) {
...
Statistics.regression = {
    linear: function (listX, listY) {
        if (listX.length != listY.length)
            throw 'Two data sets must have the same length';
        ...
        let denominator = sumX * sumX - listX.length * sumX2, numerator = sumX * sumY - sumXY * listX.length, A = (sumX * sumY - sumXY * listX.length) / denominator, B = (sumXY * sumX - sumX2 * sumY) / denominator, R = -numerator / Math.sqrt(listX.length * sumX2 - sumX * sumX) / Math.sqrt(listX.length * sumY2 - sumY * sumY), R2 = R * R;
    ...
    exponential: function (listX, listY) {
        ...
        let raw = Statistics.regression.linear(listX, listYnew), A = Math.exp(raw.b), B = Math.exp(raw.a);

...
export class Solver {
    static sim2 = function(matrix: number[][]) {  // simultaneous equation, 2 unknowns. size of matrix is 2x3
    ...
    static root = function(fx: Function, fxprime: Function, x0: number=0, method: string = 'newton', tol: number=1e-7) {
        let count = 0;
    ...
    static quadraticEqn = function(a: number, b: number, c: number, complex: boolean=false) {
        if(complex) {
    ...
    static cubicEqn = function(a: number, b: number, c: number, d: number, complex=false) {
        let d0 = b*b - 3*a*c,
    ...
    static ODE = function(mode: string = 'Euler', func: Function, x0: number, step: number = 1/60) {
        if(mode == 'Euler') {
            return x0+step*func(x0);
        }
        if(mode == 'RK4') {
            let k1 = step*func(x0),
                k2 = step*func(x0+k1/2),
                k3 = step*func(x0+k2/2),
                k4 = step*func(x0+k3);

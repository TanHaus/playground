 ...
 */
Finance.tvm = (find, ...otherArguments) => {
    let n, ir, pv, pmt, fv, rate, discount, pmt_r;
    switch (find) {
        case 'n':
 ...
 */
Finance.npv = (cf0, list_cf, list_freq, discountRate) => {
    ...
    if (discountRate != 0) {
        for (let i = 0; i < list_cf.length; i++) {
            let cf = list_cf[i], freq = list_freq[i];
 ...
 * @param {number[]} list_freq - List of frequencies
 */
Finance.irr = (cf0, list_cf, list_freq) => {
    /**
...
Finance.mirr = (cf0, list_cf, list_freq, wacc) => {
};
Finance.payback = (cf0, list_cf, list_freq) => {
};
Finance.discountedPayback = (cf0, list_cf, list_freq, wacc) => {
};
export { Finance };
